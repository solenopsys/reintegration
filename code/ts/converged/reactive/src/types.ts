...
type MapFunction<T = unknown, R = unknown> = (
	value: T,
	index: FunctionMaybe<number>,
) => R;

type MapValueFunction<T = unknown, R = unknown> = (
	value: Indexed<T>,
	index: FunctionMaybe<number>,
) => R;
...
type TryCatchFunction<T = unknown> = ({
	error,
	reset,
}: { error: Error; reset: DisposeFunction }) => T;

type UntrackFunction<T = unknown> = () => T;

type UntrackedFunction<Arguments extends unknown[], T = unknown> = (
	...args: Arguments
) => T;
...
type Callable<T extends CallableFunction> =
	| (T & { call: CallableFunctionCall<T> })
	| { call: CallableFunctionCall<T>; apply?: never };

type CallableFunction = (...args: any[]) => any;

type CallableFunctionCall<T extends CallableFunction> = (
	thiz: any,
	...args: Parameters<T>
) => ReturnType<T>;
